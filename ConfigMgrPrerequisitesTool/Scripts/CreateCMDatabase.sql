DECLARE  @SecondDataDrive     nchar(1) = N''
        ,@ConfigMgrDBRecoveryModel nvarchar(max)
        --,@InitialLogFileSize  nvarchar(50) = N'100MB'
        ,@PriDataFileGrowth   nvarchar(50) = N'512MB'
        ,@LogFileGrowth       nvarchar(50) = N'512MB';
...
IF CAST((LEFT(@InitialDataFileSize,LEN(@InitialDataFileSize)-2) * 1024) AS int) < @MdlDtaFlSze
BEGIN
    SET @InitialDataFileSize = CAST(@MdlDtaFlSze AS nvarchar(48))+N'KB';
END;

IF (CAST((LEFT(@InitialLogFileSize,LEN(@InitialLogFileSize)-2) * 1024) AS int)) < @MdlLogFlSze
BEGIN
...
/***********************************************************************************************************************************************
    Set Configuration Manager database recovery model:
***********************************************************************************************************************************************/
CMRecoveryModel:
SET @ConfigMgrDBRecoveryModel = '
	USE CM_'+@CMSiteCode+'
	ALTER DATABASE CM_'+@CMSiteCode+' SET RECOVERY SIMPLE
'
BEGIN TRY
	EXECUTE SP_EXECUTESQL @ConfigMgrDBRecoveryModel
	GOTO ScriptSuccess;
END TRY
BEGIN CATCH
    GOTO ExitError;
